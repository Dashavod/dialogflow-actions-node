...
const retrievalWithAugmentation = async (question, context) => {
  try {
    ...
    const alternativeRes = await openaiComplete(promptWithContext + "\n" + res + "\n And create new alternative answer based on the context below.");
    
    return [res.replace('Answer: ', ''), alternativeRes.replace('Answer: ', '').replace('Alternative','')]
  } catch (err) {
